...
// uint32_t intervalwheg = 2000;
//
// uint32_t toggletimer = 0;
...
void TurtleBot3Core::begin(const char* model_name)
{
  ...
  dxl_slave.addControlItem(ADDR_GOAL_POSITION, control_items.goal_position);
...
void TurtleBot3Core::run()
{
  ...
  rc100_wheg = controllers.getRCdataWheg();
  // Serial.println(rc100_wheg);
...
if (millis()-pre_time_to_control_motor >= 500)
{
  motor_driver.set_wheg_position(rc100_wheg);
}
