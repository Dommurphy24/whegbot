...
ParamForSyncWriteInst_t sync_write_param2;
...
bool Turtlebot3MotorDriver::init(void)
{
  ...
  sync_write_param2.id_count = 1;
  sync_write_param2.xel[0].id = 3;
...
bool Turtlebot3MotorDriver::set_torque(bool onoff)
{
  ...
  sync_write_param2.addr = 64;
  sync_write_param2.length = 1;
  sync_write_param2.xel[0].data[0] = onoff;

  if(dxl.syncWrite(sync_write_param) == true && dxl.syncWrite(sync_write_param2) == true){
    ret = true;
    torque_ = onoff;
  }
...
bool Turtlebot3MotorDriver::get_torque()
{
  if(dxl.readControlTableItem(TORQUE_ENABLE, left_wheel_id_) == true
    ...
    && dxl.readControlTableItem(TORQUE_ENABLE, 3) == true){
    torque_ = true;
  }else{
    torque_ = false;
  }
...
void Turtlebot3MotorDriver::set_wheg_position(bool wheg_status)
{

if (wheg_status == true)
{
  dxl.setGoalPosition(3, 1800);
}
else
{
  dxl.setGoalPosition(3, 50);
}
// Serial.println(wheg_status);
}
